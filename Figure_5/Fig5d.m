...
% intrinsic_matrix_inferred: stimulus and action decoder output correlation computed on ongoing activity. This is the same is the outlined region in Figure 5c
% evoked_matrix_inferred: stimulus and action decoder output correlation computed on evoked activity.
% r_null_inf: Correlation of the evoked responses between the first and second half of data (serves as the null distribution for comparison)

%% correlate vectorized evoked and intrinsic correlation matrices

r_all = nan(size(evoked_matrix_inferred,1),1);
for subi = 1:size(evoked_matrix_inferred,1)
    a = evoked_matrix_inferred(subi,:)';
    b = intrinsic_matrix_inferred(subi,:)';    
    r_all(subi) = corr(a,b);    
end
...
b = r_null_inf; %mean across-trial consistency of correlation of evoked responses
